...
:- ['/var/lib/myfrdcsa/codebases/minor/free-life-planner/lib/flux/swipl_flux/flux_swi'].
:- ['/var/lib/myfrdcsa/codebases/minor/free-life-planner/lib/calendaring/calendaring'].
...
poss_julian(go_meet(P,X,T),Z) :-
	...
	( X1 = X -> (julian:compare_time(T1,<,T) ; julian:compare_time(T1,=,T))
	  ;
	 distance(X1,X,D), time_add_julian(T1,ns(D),T2), (julian:compare_time(T2,<,T) ; julian:compare_time(T2,=,T))).
...
add_state_from_module(Module,ZBefore,ZAfter) :-
	pred_for_m(Module,AssertionList),
	%% FIXME: verify Assertions is a list of fluents
	see(AssertionList),
	true.
	%plus_(ZBefore,AssertionList,ZAfter).
...
init(Z0) :-
	...
	add_state_from_module(fluents,Za,Z0).


main :- init(Z0), see(Z0),
	%% plan(schedule_plan, Z0, S),
	%% writeln(S),
	true.
