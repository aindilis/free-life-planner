...
%% FIXME: datetime(X,Y) is not X * C + Y.  get a proper conversion to
%% ns from datetime(X,Y) using Julian, replace getDatetime function
%% and atTime_julian where it says 86400000000000.  Maybe using
%% form_time(unix(TimeStamp), Datetime) and then some multiplication
%% by unit conversion between seconds and nanoseconds, and adjustment
%% for nanoseconds value.  Also fix additions of days, to be like the
%% Perl where it can only add durations if it has the specific days or
%% datetimes or whatnot.  Basically any place that says 86400000000000
%% is suspect.
...
time_add_julian_ns(T1,ns(Nanoseconds),T2) :-
	T2 is T1 + Nanoseconds.
...
getDatetime(T,Datetime) :-
	Day1 is floor(T // 86400000000000),
	Ns1 is T mod 86400000000000,
	Datetime = datetime(Day1,Ns1).
...
atTime_julian(Orig,Time) :-
	...
	Time is (Day1 * 86400000000000) + Ns1.
...
main :- init(Z0), see(Z0), plan(schedule_plan, Z0, S),
	writeln(S),
	translate_go_meet_list(S,Result).
...
test_translate :-
	translate_go_meet_list([go_meet(3,2,4962853800000000000),go_meet(1,2,4962859200000000000),go_meet(2,3,4962861900000000000)],Result),
	writeln(Result).

translate_go_meet_list(List,Result) :-
	member(Item,List),
	translate_go_meet(Item,ItemResult),
	writeln(ItemResult),
	fail.
translate_go_meet_list(List,Result).

translate_go_meet(Item,ItemResult) :-
	Item = go_meet(N1,N2,Ns),
	getDatetime(Ns,Datetime),
	form_time(unix(TimeStamp), Datetime),
	%% see http://www.swi-prolog.org/pldoc/man?section=timedate
	stamp_date_time(TimeStamp, ItemResult, 'UTC').

%% FIXME: Good idea to do a sanity check in Perl datetime libraries,
%% owing to SWI Prolog's reluctance to fully support date time stuff
